  ...
  function WxShare(json) {
    ...
    $.get("/wx/GetCustomerId", function (r) {
        if (data.link.indexOf("?") > 0) {
            data.link += "&FromCustomerId=" + r + "&LevelId=" + LevelId + "&vt="
        } else {
            data.link += "?FromCustomerId=" + r + "&LevelId=" + LevelId + "vt="
        }
    });
...
!(function () {
    ...
    $(window).on('load orientationchange', function (e) {
        ...
        function fn1() {    // 显示首页
          setTimeout(function(){
            $info.hide(0).addClass('hide');
            $main.removeClass('hide').show(0);
          },50);
        }
        function fn2() {    // 显示提示
          setTimeout(function(){
            $info.removeClass('hide').show(0);
            $main.hide(0).addClass('hide');
          },50);
        }
